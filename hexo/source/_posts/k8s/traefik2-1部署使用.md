---
title: traefik2.1部署使用
date: 2020-03-23 11:13:18
tags:
- k8s
- traefik
categories: k8s
...
![traefik architecture](traefik2-1部署使用/traefik-architecture.png)
  ...
  template:
    ...
    spec:
      ...
      - image: traefik:2.1.1
        ...
        # 邮箱配置
        - --certificatesResolvers.ali.acme.email=1625317303@qq.com
        # 保存 ACME 证书的位置
...
![1584933592416](traefik2-1部署使用/1584933592416.png)
...
spec:
  entryPoints:
    - web
  routes:
  - match: Host(`who.qh.com`) && PathPrefix(`/notls`)
    kind: Rule
    services:
    - name: whoami
      port: 80
...
![1584942456583](traefik2-1部署使用/1584942456583.png)

在 `IngressRoute` 对象中我们定义了一些匹配规则，这些规则在 Traefik 中有如下定义方式：

![traefik route matcher](traefik2-1部署使用/traefik-route-matcher.png)

如果我们需要用 HTTPS 来访问我们这个应用的话，就需要监听 `websecure` 这个入口点，也就是通过 443 端口来访问，同样用 HTTPS 访问应用必然就需要证书，这里我们用 `openssl` 来创建一个自签名的证书：

```shell
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout who.qh.com.key -out who.qh.com.crt -subj "/CN=who.qh.com"
```
...
$ kubectl create secret tls who-tls --cert=who.qh.com.crt --key=who.qh.com.key
...
![1584944142728](traefik2-1部署使用/1584944142728.png)
...
![traefik wildcard domain](traefik2-1部署使用/traefik-who-wildcard-domain.png)traefik wildcard domain
...
![traefik middleware overview](traefik2-1部署使用/traefik-middleware-overview.png)traefik middleware overview
...
![canary deployment](traefik2-1部署使用/traefik-canary-demo.jpg)canary deployment
...
![traefik wrr demo](traefik2-1部署使用/traefik-wrr-demo.png)traefik wrr demo

## 流量复制

除了灰度发布之外，Traefik 2.0 还引入了流量镜像服务，是一种可以将流入流量复制并同时将其发送给其他服务的方法，镜像服务可以获得给定百分比的请求同时也会忽略这部分请求的响应。

![traefik mirror](traefik2-1部署使用/traefik-mirror.png)traefik mirror
...
这个时候我们在浏览器中去连续访问4次 `mirror.qikqiak.com` 可以发现有一半的请求也出现在了 `v2` 这个服务中：![traefik mirror demo](traefik2-1部署使用/traefik-mirror-demo.png)
...
![traefik-tcp-mongo-1](traefik2-1部署使用/traefik-tcp-mongo-1.png)traefik-tcp-mongo-1
